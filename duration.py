...
def set_hist_params(ax):
    ...
    ax.tick_params(axis='both', which='both', labelsize='small', rotation=45)
    ax.xaxis.set_ticks(arange(10000, 11001, 100))
    ax.yaxis.set_ticks(arange(0, 76, 25))
...
if __name__ == '__main__':
    ...
    outer = gridspec.GridSpec(1, 1, wspace=0.4, hspace=0.3)
    ...
    for mem in aws_mem.keys():
        ...
        aws_was[mem] = convert_to_list(tmp, 'duration')
    ...
    for mem in gcf_mem.keys():
        ...
        gcf_was[mem] = convert_to_list(tmp, 'duration')
    ...
    for mem in ibm_mem.keys():
        ...
        ibm_was[mem] = convert_to_list(tmp, 'duration')

    for i in ibm_was.keys():
        print_stats('ibm%d' % i, ibm_was[i])

    bins = linspace(10000, 11000, 100)

    # aws_c = gridspec.GridSpecFromSubplotSpec(6, 1, subplot_spec=outer[0], hspace=.0)
    gcf_c = gridspec.GridSpecFromSubplotSpec(1, 1, subplot_spec=outer[0], hspace=.0)
    ...
    plt.gcf().subplots_adjust(right=0.92, left=0.13, top=0.84, bottom=0.4)
